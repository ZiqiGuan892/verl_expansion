...
| 首个同步后端 | 服务器为 Ascend/NPU 时使用插件 `multitask_hccl`（继承原生 HCCL，适配 communicator 销毁，并在严格校验模式记录 Actor source manifest）；训练端与接收端在 backend 初始化前配置 `rebuild_group=True`。其他后端不能据此自动算作通过 |
