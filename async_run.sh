...
#
# 兼容较旧 Bash；只启用 errexit 和 nounset。
set -eu
set -x

# 此文件通过 bash 执行，不通过 source 加载；用 $0 定位脚本。
...
# 在切换目录前固定相对插件路径；driver 与 Ray 子进程均使用绝对路径。
...
# 需要进入包含 verl/ Python 包的仓根，不是包内的 trainer/experimental 所在目录。
[ -f "${VERL_SOURCE_ROOT}/verl/experimental/fully_async_policy/fully_async_main.py" ] || {
    echo "VERL_SOURCE_ROOT 不是有效的源码仓根: ${VERL_SOURCE_ROOT}" >&2
    echo "该目录下应存在 verl/experimental/fully_async_policy/fully_async_main.py" >&2
    exit 1
}
...
set -eu
# 环境脚本可能改变工作目录，加载完成后恢复到原生 verl 仓根。
cd "${VERL_SOURCE_ROOT}"
...
case "${TASK_NAME}" in
    gsm8k)
        ...
        TEST_FILE="${TEST_FILE:-/workspace/l00839669/datasets/gsm8k/train.parquet}"
...
# 不依赖管道选项；立即保存各进程的退出码，避免 tee 掩盖训练失败。
set +e
